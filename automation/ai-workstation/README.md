...
- Hermes default model: `qwen3.8-27b-uncensored-q6_k` through local LiteLLM (`http://127.0.0.1:4004/v1`) and llama.cpp service `llama-qwen38-uncensored-q6k.service` on port `11446`. Hermes config sets `model.context_length=131072` and `model.ollama_num_ctx=131072`. Q4_K_M remains installed as the disabled rollback. As of 2026-09-03, Q6_K is intentionally stopped while `qwen3.8-27b-uncensored-orcarouter-q4_k_m` is resident for user testing; rollback is to stop `llama-qwen38-orcarouter-uncensored-q4km.service` and start `llama-qwen38-uncensored-q6k.service`.
...
- Hermes model alias source: `qwen3-coder:30b-a3b-q8_0`; aliases exist for `hermes-qwen3-coder:30b-64k`, `hermes-qwen3-coder:30b-128k`, and `hermes-qwen3-coder:30b-256k`. A temporary 2026-09-02 Flash-Next cleanup changed these routes to `keep_alive: 0` for unload verification; owner-requested cleanup restored the prior `keep_alive: 30m` values.
...
- Qwen3.8-Flash-Next status: not promoted on 2026-09-02. A side-by-side `llama-rocm-10.0-qwen38-flash-next` toolbox using llama.cpp build `10672` / commit `590ac45bc` was created and the AtomicChat `AD-4.27bpw-Q4_K_M-M64` split GGUF was downloaded and verified under `/mnt/ai/models/qwen38-flash-next-ad-4.27bpw-q4km-m64`. Isolated load attempts were killed by host RAM OOM before health. Owner-requested cleanup removed the downloaded model directory, temporary service, LiteLLM route, test toolbox, pulled image, and Flash attempt backup folders; Q6_K was verified active and healthy afterward. Do not retry without an owner-approved memory-layout change or a materially smaller higher-confidence candidate.
- Qwen3.8 OrcaRouter uncensored test: `qwen3.8-27b-uncensored-orcarouter-q4_k_m` uses `llama-qwen38-orcarouter-uncensored-q4km.service` on `http://127.0.0.1:11448/v1`, backed by `/mnt/ai/models/qwen38-orcarouter-uncensored-q4km/Qwen3.8-27B-Uncensored-OrcaRouter-Q4_K_M.gguf`. It was downloaded from `chimingw/Qwen3.8-27B-Uncensored-OrcaRouter-GGUF` revision `58ebd123013160600229eda180b5b17f3fb7af9d` and verified against SHA-256 `3445102e9cde5d562508642c100a2f5ac3368a5a3f748442811d7a95daee3bec`. Direct and LiteLLM smokes returned `ORCAROUTER_OK`. Use profile `qwen38-27b-uncensored-orcarouter-q4_k_m-web-terminal-128k` for browser testing.
...
| `qwen38-27b-uncensored-orcarouter-q4_k_m-web-terminal-128k` | OrcaRouter uncensored Q4_K_M test profile |
...
- Provisioned toolbox containers include `llama-rocm-7.2.2`, refreshed `llama-rocm-7.14`, `llama-rocm-7.14-q4`, and `llama-rocm-10.0-qwen38-test`.
- Qwen3.8 BF16 runtime image: `docker.io/kyuz0/amd-strix-halo-toolboxes:rocm-7.14`
- Qwen3.8-Flash-Next experimental runtime image tested on 2026-09-02: `docker.io/kyuz0/amd-strix-halo-toolboxes:rocm-10.0-qwen-3.8-flash-next` (`sha256:d2067225019f61d39c6531322fff8469d67a9a4457c83ed50945b8dffdd818be`). The local toolbox and pulled image were removed during owner-requested cleanup after the failed load.
