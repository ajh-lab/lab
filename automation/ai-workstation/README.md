...
- Qwen3.8 Flash-Next Unsloth `UD-IQ3_XXS` 131k retest: on 2026-09-22, a transient 131k direct route on port `11460` used the existing 16k service flags except `-c 131072`. It reached health after about 5 minutes 49 seconds and used about 56.6 GB VRAM after benchmark, but still dropped to about 432 MiB available RAM and about 43.4 GiB swap during load. Direct text and forced-tool smokes passed, but forced tool-call completion was only 5.71 tok/s, and the controlled telemetry benchmark scored 5/18 first and 7/18 repaired. Do not promote 131k `UD-IQ3_XXS` as a Cline/DSH default. Artifacts: `docs/ai-workstation/qwen38-performance/results/2026-09-22-flashnext-unsloth-ud-iq3xxs-131k-quality.md`; raw remote run folder `/home/helios/benchmarks/qwen38-flashnext-ud-iq3xxs-131k-quality-20260922T145755Z`.
- Current Flash-Next manual test default as of 2026-09-22: `qwen3.8-flash-next-uncensored-orcarouter-iq4_xs` is served through transient `llama-qwen38-flashnext-iq4xs-64k-manual.service` / port `11454` with 64k context, full offload, the matching OrcaRouter F16 multimodal projector, and the safer KV-cache reduction. The current flags are `--mmproj /mnt/ai/models/qwen38-flashnext-orcarouter-uncensored-iq4xs/mmproj-Qwen3.8-Flash-Next-Uncensored-F16.gguf -ngl 999 -c 65536 -b 1024 -ub 256 -fa on -ctk q4_0 -ctv q8_0 --parallel 1 --reasoning off --temp 0 --repeat-penalty 1.0 -lm dio --no-warmup --no-webui`. The 64k route reports `n_ctx=65536`, keeps vision/video enabled, and passed internal LiteLLM smoke with `qwen3.8-flash-next` advertising `max_input_tokens=65536`. This replaced the 131k q4k/q8v route after interactive Cline/DSH use showed swap-heavy stalls around 58k live context. Keep all large model services disabled for autostart; this is a supervised manual default. `qwen3.8-27b-uncensored-orcarouter-q4_k_m` remains the lower-VRAM quality rollback and `UD-IQ3_XXS` remains the safer Flash-Next fallback.
...
- Hermes Discord route note: the legacy profile name `qwen38-27b-uncensored-q6_k-discord-128k` is still used by the `@Helios` Discord bot path. It currently points at `qwen3.8-flash-next-uncensored-orcarouter-iq4_xs` through LiteLLM with 64k context metadata after the 2026-09-22 reduction from 131k. Backups include `/home/helios/.hermes/profiles/qwen38-27b-uncensored-q6_k-discord-128k/config.yaml.bak-flashnext-route-20260907-133709`, `/home/helios/.hermes/backups/promote-flashnext-iq4xs-20260907-180340/discord-profile-config.yaml`, and `/home/helios/.hermes/backups/restore-q4-after-iq3m-131k-20260907-221857/discord-profile-config.yaml`. Actual outbound delivery to `discord:helios` returned `sent` on 2026-09-07, and the profile smoke returned `DISCORD_ROUTE_OK` then.
...
- Qwen3-Coder-Next Uncensored Heretic GGUF test files were downloaded on 2026-09-22 for Cline/DSH follow-up testing. Q8_0 is `/mnt/ai/models/qwen3-coder-next-uncensored-heretic-q8_0/Qwen3-Coder-Next-Uncensored-Heretic-Q8_0.gguf` (`sha256 d516e8ede3a8477e39e5fe8b92ce79bf428aeeb0a4f0cf8b5c65795dd5ff9d38`, about 80G). Q6_K is `/mnt/ai/models/qwen3-coder-next-uncensored-heretic-q6_k/Qwen3-Coder-Next-Uncensored-Heretic-Q6_K.gguf` (`sha256 7b82235501ed8835c0a2137185bf628ce8d45fc4dea3852e7dd2a2047cdfc245`, about 62G). Standard smoke testing at 262k and 128k context used one resident model at a time on port `11460`; Q6_K reached about 35 tok/s on the coding prompt with 66.6-69.0 GiB VRAM after health and passed the forced tool-call smoke at both contexts. Q8_0 reached about 31.5 tok/s but used 84.5-86.9 GiB VRAM. Results are documented in `docs/ai-workstation/qwen38-performance/results/2026-09-22-qwen3-coder-next-heretic-q8-q6-context-comparison.md`. Follow-up 128k controlled Python telemetry quality runs passed forced tool calling for both quants, but both Q6_K and Q8_0 scored only 11/18 first and 14/18 repaired. Q8_0 produced 34.88 tok/s first / 33.12 tok/s repair and used about 88.6 GB VRAM in the quality run, so higher quant did not fix the quality gap and used materially more VRAM. Do not promote this model family as the default from these results. Detailed quality reports: `docs/ai-workstation/qwen38-performance/results/2026-09-22-qwen3-coder-next-heretic-q6-131k-quality.md` and `docs/ai-workstation/qwen38-performance/results/2026-09-22-qwen3-coder-next-heretic-q8-131k-quality.md`.
...
- Qwen3.8 Flash-Next OrcaRouter IQ4_XS MTP rejection: on 2026-09-22, a transient 64k route on port `11456` added `--spec-type draft-mtp` with `/mnt/ai/models/qwen38-flashnext-orcarouter-uncensored-iq3m/Qwen3.8-Flash-Next-Uncensored-MTP-draft.gguf` while keeping the current IQ4_XS q4k/q8v flags. It passed direct health and forced tool-call smoke and improved decode speed to 34.73 tok/s first generation / 26.96 tok/s repair generation, with short coding generation reaching 42.41 tok/s. However, the controlled Python telemetry repaired score regressed from the no-MTP 17/18 baseline to 14/18, and a 14.3k-token native prompt still prefilling at only 75.73 prompt tok/s showed MTP does not fix long prompt processing. Do not promote this exact MTP config as the Cline/DSH default. Artifacts: `docs/ai-workstation/qwen38-performance/results/2026-09-22-flashnext-iq4xs-64k-mtp-test.md`; raw remote run folder `/home/helios/benchmarks/qwen38-flashnext-iq4xs-mtp-20260922T124105Z`.
- Qwen3.8 Flash-Next OrcaRouter IQ4_XS 32k no-MTP abort: on 2026-09-22, a transient 32k route on port `11460` used the current no-MTP IQ4_XS vision flags with `-c 32768`, `-ctk q4_0`, and `-ctv q8_0`. It had not reached health when the run was stopped; after roughly 2.5 minutes `/health` still returned `Loading model`, available RAM had fallen to about 634 MiB, swap had climbed to about 37 GiB, and VRAM was about 69.3 GB. The service was stopped and cleanup verification showed no resident llama process, `ollama ps` empty, and VRAM back to baseline. Treat this as an aborted safety run, not proof that 32k cannot load; it does show that context reduction to 32k does not by itself avoid IQ4_XS load-time host memory pressure on the 96 GiB VRAM / 31 GiB Linux RAM split. Artifacts: `docs/ai-workstation/qwen38-performance/results/2026-09-22-flashnext-iq4xs-32k-nomtp-load-abort.md`; raw remote run folder `/home/helios/benchmarks/qwen38-flashnext-iq4xs-32k-nomtp-quality-20260922T1406Z`.
- Follow-up Qwen3.8 Flash-Next OrcaRouter IQ4_XS 32k no-MTP quality retest: the same 32k route reached health after about 3 minutes 47 seconds when given a longer supervised load window. Preflight was clean with no resident model and baseline VRAM around 754 MB. Load still dipped to about 420 MiB available RAM and about 44.1 GiB swap, then recovered to about 11 GiB available RAM at health. Text and forced-tool smokes passed, but the controlled Python telemetry benchmark scored 13/18 first and only 14/18 repaired at 25.33 tok/s first / 23.34 tok/s repair. Do not promote 32k as the default: it can load, but it did not improve speed, did not avoid startup pressure, and regressed repaired quality versus the earlier 64k IQ4_XS 17/18 baseline. Artifacts: `docs/ai-workstation/qwen38-performance/results/2026-09-22-flashnext-iq4xs-32k-nomtp-quality-retest.md`; raw remote run folder `/home/helios/benchmarks/qwen38-flashnext-iq4xs-32k-nomtp-quality-20260922T142410Z`.
...
- ai-workstation monitoring: `node-exporter.service` exposes host metrics on `0.0.0.0:9100`; it also reads the node_exporter textfile directory at `/home/helios/.local/share/node-exporter/textfile_collector`, where `ai-workstation-process-memory.timer` writes the current top process RSS metrics. `ai-workstation-gpu-exporter.service` exposes ROCm/sysfs GPU metrics on `0.0.0.0:9101`. Prometheus scrapes both and the `LiteLLM` Grafana dashboard includes an `AI Workstation Health` section.
