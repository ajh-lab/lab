...
- Qwen3.8 Flash-Next OrcaRouter IQ4_XS 32k no-MTP rejection: on 2026-09-22, a transient 32k route on port `11460` used the current no-MTP IQ4_XS vision flags with `-c 32768`, `-ctk q4_0`, and `-ctv q8_0`. It did not reach health; after roughly 2.5 minutes `/health` still returned `Loading model`, available RAM had fallen to about 634 MiB, swap had climbed to about 37 GiB, and VRAM was about 69.3 GB. The service was stopped and cleanup verification showed no resident llama process, `ollama ps` empty, and VRAM back to baseline. Treat this as evidence that context reduction to 32k does not by itself fix IQ4_XS load-time host memory pressure on the 96 GiB VRAM / 31 GiB Linux RAM split. Artifacts: `docs/ai-workstation/qwen38-performance/results/2026-09-22-flashnext-iq4xs-32k-nomtp-load-abort.md`; raw remote run folder `/home/helios/benchmarks/qwen38-flashnext-iq4xs-32k-nomtp-quality-20260922T1406Z`.
