...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.18.0 / 2026.7.1` as of 2026-07-07)
    ...
    - DeepSeek cloud profiles: `deepseek-v4-flash` and `deepseek-v4-pro` use LiteLLM at `http://127.0.0.1:4000/v1`, aliases `/home/helios/.local/bin/deepseek-v4-flash` and `/home/helios/.local/bin/deepseek-v4-pro`, and profile env files under `/home/helios/.hermes/profiles/<profile>/.env`. `deepseek-v4-flash` is the normal paid profile; `deepseek-v4-pro` is reserved for complex planning, review, difficult debugging, or failed-card recovery. The older `deepseek` profile remains as a compatibility alias to `deepseek-v4-flash`. On 2026-07-08, both `hermes -p deepseek-v4-flash -z ...` and `hermes -p deepseek-v4-pro -z ...` returned successfully through LiteLLM. This replaced legacy `deepseek-chat`, which DeepSeek API docs list as a compatibility alias scheduled for deprecation.
...
- Hermes Kanban board: `lab-monitoring` (`Lab Monitoring`), originally created with the legacy `deepseek` profile. New DeepSeek-backed cards should prefer the model-named `deepseek-v4-flash` profile unless the task needs `deepseek-v4-pro`.
