...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.18.0 / 2026.7.1` as of 2026-07-07)
    - Default local model: `hermes-qwen3-coder:30b-64k` via local LiteLLM OpenAI-compatible endpoint (`http://127.0.0.1:4000/v1`), which forwards to Ollama and emits Prometheus usage metrics.
    - Long-context qwen3-coder profiles: `qwen3-coder-128k` and `qwen3-coder-256k` use aliases `hermes-qwen3-coder:30b-128k` and `hermes-qwen3-coder:30b-256k` through LiteLLM. They are backed by the same `qwen3-coder:30b-a3b-q8_0` weights with larger Ollama `PARAMETER num_ctx` values. On 2026-07-09, both profiles returned through Hermes and LiteLLM, and Prometheus showed `requested_model="hermes-qwen3-coder:30b-128k"` and `requested_model="hermes-qwen3-coder:30b-256k"` under job `litellm-ai-workstation`. Ollama allocated about 42.7 GiB total for 128k (`12.0 GiB` KV cache) and about 55.0 GiB total for 256k (`24.0 GiB` KV cache), fully offloading 49/49 layers to ROCm. Keep the default at 64k unless the task needs repository-scale context; use `qwen3-coder-256k` when maximum native context is useful and latency is acceptable.
    ...
    - Runtime model aliases point to `qwen3-coder:30b-a3b-q8_0`; the default alias uses `PARAMETER num_ctx 65536`, while test long-context aliases use `131072` and `262144`. Matching Hermes profile configs set `model.context_length` and `model.ollama_num_ctx` to the same values.
