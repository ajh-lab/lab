...
When Codex credits are unavailable, Hermes DeepSeek profiles can be used for Kanban work. The DeepSeek provider key is stored in OpenBao at `secret/homelab/providers/deepseek`, field `api_key`, and the ai-workstation Hermes profile `.env` files are populated with `DEEPSEEK_API_KEY` as the controlled runtime fallback. As of 2026-07-08, the model-named Hermes profiles `deepseek-v4-flash` and `deepseek-v4-pro` route through local LiteLLM at `http://127.0.0.1:4000/v1`, so token/request usage appears in Grafana alongside local qwen3-coder usage. The local `default` Hermes profile should continue to use the ai-workstation-hosted qwen3-coder model for routine/local work. Use `deepseek-v4-flash` for normal paid DeepSeek work and `deepseek-v4-pro` only for complex planning, review, difficult debugging, or failed-card recovery. Do not use the old `deepseek` profile name; it was removed to avoid ambiguous model selection.
...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.18.0 / 2026.7.1` as of 2026-07-07)
    - Default local model: `hermes-qwen3-coder:30b-256k` via local LiteLLM OpenAI-compatible endpoint (`http://127.0.0.1:4000/v1`), which forwards to Ollama and emits Prometheus usage metrics. The default Hermes config sets `model.context_length=262144` and `model.ollama_num_ctx=262144`.
    ...
    - DeepSeek cloud profiles: `deepseek-v4-flash` and `deepseek-v4-pro` use LiteLLM at `http://127.0.0.1:4000/v1`, aliases `/home/helios/.local/bin/deepseek-v4-flash` and `/home/helios/.local/bin/deepseek-v4-pro`, and profile env files under `/home/helios/.hermes/profiles/<profile>/.env`. `deepseek-v4-flash` is the normal paid profile; `deepseek-v4-pro` is reserved for complex planning, review, difficult debugging, or failed-card recovery. The older `deepseek` profile and `/home/helios/.local/bin/deepseek` alias were removed on 2026-07-08 to avoid confusion. On 2026-07-08, both `hermes -p deepseek-v4-flash -z ...` and `hermes -p deepseek-v4-pro -z ...` returned successfully through LiteLLM. This replaced legacy `deepseek-chat`, which DeepSeek API docs list as a compatibility alias scheduled for deprecation.
    ...
    - LiteLLM local proxy: user systemd service `litellm-ollama-proxy.service`, config `/home/helios/.config/litellm/config.yaml`, venv `/home/helios/.local/share/litellm/venv`, bound to `127.0.0.1:4000`. It maps model names `hermes-qwen3-coder:30b-64k`, `hermes-qwen3-coder:30b-128k`, and `hermes-qwen3-coder:30b-256k` to Ollama at `http://127.0.0.1:11434`, maps `deepseek-v4-flash` and `deepseek-v4-pro` to DeepSeek using `DEEPSEEK_API_KEY`, and exposes Prometheus metrics at `http://127.0.0.1:4000/metrics/`. Its wrapper `/home/helios/.local/bin/litellm-openbao-env` falls back to `/home/helios/.hermes/profiles/deepseek-v4-flash/.env` or `/home/helios/.hermes/profiles/deepseek-v4-pro/.env`; do not reintroduce the removed legacy `deepseek` profile path.
    ...
    - Local model benchmark harness: `automation/ai-workstation/scripts/benchmark-hermes-models.py` runs dependency-free on the ai-workstation and can test either LiteLLM (`http://127.0.0.1:4000/v1`) or Ollama (`http://127.0.0.1:11434`). Use it before changing default model aliases, context windows, or profile routing. Store ad hoc JSONL outputs under `/tmp/` on the workstation.
