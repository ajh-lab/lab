...
When Codex credits are unavailable, the Hermes `deepseek` profile can be used for Kanban work. The DeepSeek provider key is stored in OpenBao at `secret/homelab/providers/deepseek`, field `api_key`, and the ai-workstation Hermes profile `.env` is populated with `DEEPSEEK_API_KEY` as the controlled runtime fallback. As of 2026-07-08, the `deepseek` Hermes profile is routed through local LiteLLM at `http://127.0.0.1:4000/v1` using model `deepseek-v4-flash`, so token/request usage appears in Grafana alongside local qwen3-coder usage. The local `default` Hermes profile should continue to use the ai-workstation-hosted qwen3-coder model for routine/local work, while `deepseek` is intended for harder coding, review, and research tasks.
...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.18.0 / 2026.7.1` as of 2026-07-07)
    ...
    - Secondary profile: `deepseek` uses model `deepseek-v4-flash` through LiteLLM at `http://127.0.0.1:4000/v1`, alias `/home/helios/.local/bin/deepseek`, and profile env file `/home/helios/.hermes/profiles/deepseek/.env`. On 2026-07-08, `hermes -p deepseek -z ...` returned through LiteLLM and Prometheus showed `requested_model="deepseek-v4-flash"` under job `litellm-ai-workstation`. This replaced legacy `deepseek-chat`, which DeepSeek API docs list as a compatibility alias scheduled for deprecation.
