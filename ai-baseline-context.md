...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.18.0 / 2026.7.1` as of 2026-07-07)
    - Default local model: `hermes-qwen3-coder:30b-256k` via local LiteLLM OpenAI-compatible endpoint (`http://127.0.0.1:4004/v1`), which forwards to Ollama and emits Prometheus usage metrics. The default Hermes config sets `model.context_length=262144` and `model.ollama_num_ctx=262144`.
    ...
    - Runtime model aliases point to `qwen3-coder:30b-a3b-q8_0`; aliases exist for `65536`, `131072`, and `262144` contexts. The default Hermes config now uses the `262144` alias. Matching Hermes profile configs set `model.context_length` and `model.ollama_num_ctx` to the same values.
