...
- Runtime/services (validated 2026-05-23):
  ...
  - `hermes-gateway` user systemd service active (`Hermes Agent v0.21.0 / 2026.8.31`, upstream `5b0f68b4`, validated 2026-09-01)
    - Default local model: `qwen3.8-27b-uncensored-q6_k` via the local LiteLLM OpenAI-compatible endpoint (`http://127.0.0.1:4004/v1`) and llama.cpp service `llama-qwen38-uncensored-q6k.service` on port `11446`. The default Hermes config sets `model.context_length=131072` and `model.ollama_num_ctx=131072`. Q4_K_M remains installed as the disabled rollback.
    - Hermes update recovery: the 2026-09-01 update from `v0.20.5` to `v0.21.0` created `/home/helios/.hermes/backups/pre-update-2026-09-01-223739.zip`. The prior local Kanban heartbeat/dashboard patch is parked as git stash `hermes-update-autostash-20260902-033801` and as `/home/helios/.hermes/backups/hermes-update-20260902T033737Z/local-source-changes.patch`; do not reapply it without reconciling it against the newer upstream heartbeat and stale-worker implementation.
    - Long-context qwen3-coder profiles: `qwen3-coder-30b-a3b-q8_0-full-128k` and `qwen3-coder-30b-a3b-q8_0-full-256k` use aliases `hermes-qwen3-coder:30b-128k` and `hermes-qwen3-coder:30b-256k` through LiteLLM. They are backed by the same `qwen3-coder:30b-a3b-q8_0` weights with larger Ollama `PARAMETER num_ctx` values. On 2026-07-09, both profiles returned through Hermes and LiteLLM, and Prometheus showed `requested_model="hermes-qwen3-coder:30b-128k"` and `requested_model="hermes-qwen3-coder:30b-256k"` under job `litellm-ai-workstation`. Ollama allocated about 42.7 GiB total for 128k (`12.0 GiB` KV cache) and about 55.0 GiB total for 256k (`24.0 GiB` KV cache), fully offloading 49/49 layers to ROCm. The global default was switched to `hermes-qwen3-coder:30b-256k` on 2026-07-09, then superseded by Qwen3.8 Q6_K on 2026-09-01.
    - Restricted web profile: `qwen3-coder-30b-a3b-q8_0-web-128k` prevents lightweight browser chat from running the full local-action agent loop. Its config routes through LiteLLM, sets `platform_toolsets.cli` to `web` plus `no_mcp`, disables terminal/file/code-execution/browser/computer-use and other local-action toolsets via `agent.disabled_toolsets`, sets `agent.max_turns: 4`, and sets `agent.environment_probe: false`. Use the Q6 web-terminal profile when local terminal access is required.
    - Lean Kanban worker profile: `qwen3-coder-30b-a3b-q8_0-worker-128k` uses `hermes-qwen3-coder:30b-128k` through LiteLLM, keeps only terminal/file/code-execution/todo/skills tools, disables browser/image/TTS/computer-use/delegation/cron/memory extras, sets `agent.max_turns: 28`, sets `agent.reasoning_effort: none` so Ollama does not receive unsupported thinking requests, and sets `agent.environment_probe: false` with a fixed lab workspace hint. Use this profile for focused repo implementation and validation cards. Reserve full/default or 256k profiles for cards that need broader tools or very large context.
    - Qwen3.8 BF16 profiles: `qwen38-27b-uncensored-bf16-full-128k` retains the full Hermes tool surface, while `qwen38-27b-uncensored-bf16-web-terminal-128k` provides the restricted browser-chat tool surface. The Ollama tag `qwen3.8:27b-bf16` is installed in the isolated model store `/mnt/ai/ollama-qwen38/models`, but Ollama `0.32.14` was not usable for this dense BF16 model on Strix Halo because it only offloaded 30/66 layers and was OOM-killed after spilling about 28 GiB to CPU memory. The working runtime is `llama-qwen38-bf16.service`, a user systemd service backed by the refreshed `llama-rocm-7.14` toolbox and `llama-server` on `http://127.0.0.1:11437/v1`. LiteLLM maps `qwen3.8-27b-bf16` through `http://127.0.0.1:4004/v1`.
    ...
    - Metered local profile: `qwen3-coder-30b-a3b-q8_0-metered-64k` uses `hermes-qwen3-coder:30b-64k` through LiteLLM at `http://127.0.0.1:4004/v1`. This profile was created for explicit local usage tracking tests; the default Discord-backed Hermes profile uses the same LiteLLM backend.
    - LiteLLM internal backend: user systemd service `litellm-ollama-proxy.service`, config `/home/helios/.config/litellm/config.yaml`, venv `/home/helios/.local/share/litellm/venv`, bound to `127.0.0.1:4004`. It maps model names `hermes-qwen3-coder:30b-64k`, `hermes-qwen3-coder:30b-128k`, and `hermes-qwen3-coder:30b-256k` to Ollama at `http://127.0.0.1:11434`, maps `qwen3.8-27b-bf16` to the local llama.cpp server at `http://127.0.0.1:11437/v1`, maps `deepseek-v4-flash` and `deepseek-v4-pro` to DeepSeek using `DEEPSEEK_API_KEY`, and exposes Prometheus metrics at `http://127.0.0.1:4004/metrics/`. Its wrapper `/home/helios/.local/bin/litellm-openbao-env` falls back to `/home/helios/.hermes/profiles/deepseek-v4-flash/.env` or `/home/helios/.hermes/profiles/deepseek-v4-pro/.env`; do not reintroduce the removed legacy `deepseek` profile path. LiteLLM was upgraded from `1.91.0` to `1.97.0` on 2026-08-18 after restoring `pip` in the venv; package freeze backups were written under `/home/helios/.local/share/litellm/backups/`.
    ...
    - LiteLLM validation on 2026-08-18: `litellm-ollama-proxy.service`, `litellm-lan-auth-proxy.service`, and `litellm-metrics-proxy.service` were active after the `1.97.0` upgrade; ports `4004`, `4000`, and `4001` were listening; internal `/v1/models` included `hermes-qwen3-coder:30b-256k`, `qwen3.8-27b-bf16`, and `deepseek-v4-flash`; LAN auth `/v1/models` returned HTTP 200 with the stored bearer key; metrics returned Prometheus output; and a LiteLLM completion through `qwen3.8-27b-bf16` returned `OK_UPDATED_LITELLM`. Earlier checks also validated direct OpenAI-compatible non-streaming/streaming calls, the metered qwen3-coder profile, default Hermes through LiteLLM, and request/latency/token metrics.
    - Qwen3-coder runtime aliases point to `qwen3-coder:30b-a3b-q8_0`; aliases exist for `65536`, `131072`, and `262144` contexts. Matching Hermes profile configs set `model.context_length` and `model.ollama_num_ctx` to the same values. These remain explicit profiles after the root default moved to Qwen3.8 Q6_K.
    - Local model benchmark harness: `automation/ai-workstation/scripts/benchmark-hermes-models.py` runs dependency-free on Python 3 and can test either internal LiteLLM (`http://127.0.0.1:4004/v1`) or Ollama (`http://127.0.0.1:11434`). Use it before changing default model aliases, context windows, or profile routing. Store ad hoc JSONL outputs under `/tmp/` on the workstation.
    - Qwen3.8 27B performance tuning source of truth: read `docs/ai-workstation/qwen38-performance/README.md` before changing Qwen3.8 model services, toolboxes, server flags, model routing, kernel/GTT settings, or benchmark procedure. It records the verified host/runtime baseline, rollback paths, append-only raw results, accepted and rejected experiments, and the ordered backend/batch/kernel test matrix.
    - Qwen3.8-Flash-Next preflight on 2026-09-02 stopped before any large download. Official BF16/FP8 checkpoints and quality-preserving GGUFs do not fit the current 96 GiB GPU allocation; plausible-fit GGUFs are low precision and require a side-by-side llama.cpp runtime newer than the active ROCm build `10540`.
