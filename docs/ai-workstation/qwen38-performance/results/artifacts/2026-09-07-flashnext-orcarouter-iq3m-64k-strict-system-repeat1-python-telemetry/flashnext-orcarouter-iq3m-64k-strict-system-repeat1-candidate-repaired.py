...
def _parse_rfc3339_utc(value: Any) -> datetime:
    ...
    # Check for explicit timezone and strict RFC3339 format
    # RFC3339 requires: YYYY-MM-DDThh:mm:ss[.fraction]Z or YYYY-MM-DDThh:mm:ss[.fraction]+hh:mm or -hh:mm
    # Python's fromisoformat in 3.11+ is more lenient, so we need to validate strictly.

    # Basic structure check: must have 'T' separator
    if 'T' not in value:
    ...
    if value.endswith('Z'):
        ...
        # Find the last occurrence of + or - that indicates timezone
        # It must be after the time part
        parts = value.split('T')
        if len(parts) != 2:
            raise ValueError(f"observed_at is not a valid RFC3339 string: {value}")
        time_part = parts[1]
        ...
        # Validate the offset format: must be +hh:mm or -hh:mm
        # Find the offset separator
        offset_idx = -1
        ...
        offset_str = time_part[offset_idx:]
        # Offset must be exactly 6 characters: +hh:mm or -hh:mm
        if len(offset_str) != 6:
            raise ValueError(f"observed_at timezone offset must be in format +hh:mm or -hh:mm: {offset_str}")

        if offset_str[0] not in '+-':
            raise ValueError(f"observed_at timezone offset must start with + or -: {offset_str}")

        if offset_str[3] != ':':
            raise ValueError(f"observed_at timezone offset must have colon between hours and minutes: {offset_str}")
        ...
        try:
            ...
            if not (0 <= hh <= 23):
                raise ValueError(f"observed_at timezone offset hours out of range: {hh}")
            if not (0 <= mm <= 59):
                raise ValueError(f"observed_at timezone offset minutes out of range: {mm}")
        except ValueError:
            raise ValueError(f"observed_at timezone offset is invalid: {offset_str}")
    else:
        raise ValueError("observed_at must contain an explicit timezone")
