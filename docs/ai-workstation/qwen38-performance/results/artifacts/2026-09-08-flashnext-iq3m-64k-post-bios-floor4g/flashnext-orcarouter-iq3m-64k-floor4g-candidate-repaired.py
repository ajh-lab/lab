...
def reconstruct_tracks(events, max_gap_seconds):
    ...
    normalized_events = []
    for event in events:
        if not hasattr(event, 'keys') or not hasattr(event, '__getitem__'):
        ...
        if fix not in valid_fixes:
            raise ValueError("fix must be one of the valid options")
    ...
    for asset_id, evs in assets.items():
        ...
        for ev in evs:
            ...
            if active_segment is None:
                ...
                prev_time = prev_ev["observed_at"]
                curr_time = ev["observed_at"]
                elapsed = (curr_time - prev_time).total_seconds()

                if elapsed <= 0:
                    # Timestamp is less than or equal to previous, start new segment
                    segments.append(active_segment)
                    active_segment = {
                        "asset_id": asset_id,
                        "events": [ev],
    ...
    for seg in segments:
        ...
        best_fix = min(evs, key=lambda e: fix_rank[e["fix"]])["fix"]
