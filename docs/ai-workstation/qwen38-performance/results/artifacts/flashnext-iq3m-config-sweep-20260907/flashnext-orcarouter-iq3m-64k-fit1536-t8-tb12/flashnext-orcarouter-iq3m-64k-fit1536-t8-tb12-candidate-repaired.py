...
def reconstruct_tracks(events, max_gap_seconds):
    ...
    for event in events:
        ...
        # Parse the timezone portion manually to ensure no seconds
        # The fromisoformat may accept extra precision; we need to reject offsets with seconds
        # Check if the string contains a timezone offset with seconds
        # Look for patterns like +HH:MM:SS or -HH:MM:SS
        # Also check for Z which is fine
        # We'll check the tzinfo's utcoffset for sub-minute precision
        utcoffset = dt.utcoffset()
        if utcoffset is not None:
            total_seconds = utcoffset.total_seconds()
            # Check if there are any seconds beyond minutes
            if total_seconds % 60 != 0:
                raise ValueError("observed_at timezone offset must have only hours and minutes")
