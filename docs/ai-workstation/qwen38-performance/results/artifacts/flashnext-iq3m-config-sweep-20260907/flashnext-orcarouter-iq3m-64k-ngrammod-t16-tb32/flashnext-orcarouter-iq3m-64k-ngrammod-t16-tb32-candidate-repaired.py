...
def reconstruct_tracks(events, max_gap_seconds):
    ...
    for event in events:
        ...
        # Check that the timezone offset has only hours and minutes (no seconds)
        # RFC3339 allows Z or ±HH:MM
        # datetime.fromisoformat may parse offsets with seconds in some cases
        # We need to verify the original string's timezone part
        # Check if the string ends with Z or has a valid ±HH:MM offset
        # The fromisoformat in Python 3.11+ handles Z, but we need to ensure no seconds in offset
        # Let's check the tzinfo offset
        tz_offset = dt.utcoffset()
        if tz_offset is not None:
            total_seconds = tz_offset.total_seconds()
            # Check if the offset has a seconds component
            if total_seconds != int(total_seconds):
                raise ValueError("observed_at must be a valid RFC3339 string")
            # Check that the offset is within valid range and has no seconds
            # RFC3339 offsets are ±HH:MM, so the seconds component must be 0
            # Also check that the offset doesn't have a fractional minute component
            # The offset in seconds should be a multiple of 60
