...
def _parse_rfc3339_utc(s: str) -> datetime:
    ...
    # Strict validation for RFC3339 timezone format
    # Must end with 'Z' or '+HH:MM' or '-HH:MM'
    if not (s.endswith('Z') or (len(s) >= 6 and s[-6] in '+-' and s[-3] == ':')):
        raise ValueError(f"Invalid RFC3339 timestamp format: {s}")

    # Additional check for offset format if not Z
    if not s.endswith('Z'):
        # Check that the offset part is exactly HH:MM
        offset_part = s[-5:]
        if len(offset_part) != 5 or offset_part[2] != ':':
            raise ValueError(f"Invalid RFC3339 timezone offset format: {s}")
        ...
        try:
            ...
            if not (0 <= hours <= 23 and 0 <= minutes <= 59):
                raise ValueError(f"Invalid RFC3339 timezone offset range: {s}")
        except ValueError:
            raise ValueError(f"Invalid RFC3339 timezone offset format: {s}")
