...
def reconstruct_tracks(events: Iterator[Dict[str, Any]], max_gap_seconds: float) -> List[Dict[str, Any]]:
    ...
    for i, event in enumerate(events):
        ...
        # Check for valid timezone offset format (hours and minutes only)
        if observed_at_str[-6] == ':':
            # Check if the timezone part is valid
            tz_part = observed_at_str[-6:]
            if not (tz_part[0] in '+-' and tz_part[3] == ':' and tz_part[1:3].isdigit() and tz_part[4:6].isdigit()):
                raise ValueError(f"Event {i} observed_at timezone offset must be in HH:MM format")
    ...
    for asset_id, events in asset_events.items():
        ...
        for event in events:
            ...
            if start_new:
                ...
                active_segment["times"].append(timestamp)
                # Update best fix
