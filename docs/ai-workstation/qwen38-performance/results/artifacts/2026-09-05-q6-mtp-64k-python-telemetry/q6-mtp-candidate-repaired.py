...
def _parse_rfc3339_utc(value):
    ...
    s = value.strip()
    # Minimum length for "YYYY-MM-DDTHH:MM:SS+HH:MM" is 25.
    if len(s) < 20:
        raise ValueError("invalid RFC 3339 timestamp format")
    ...
    # Validate that the offset contains only hours and minutes (seconds == 0).
    off = dt.utcoffset()
    total_seconds = int(off.total_seconds())
    if total_seconds % 3600 != 0 or total_seconds % 60 != 0:
        raise ValueError("observed_at timezone offset must be in hours and minutes")
